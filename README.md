...
| `ENTRY_ZONE_ONLY` | `true` | confirmation validates the setup; the entry is taken at the **analysed** price, not wherever the evidence happened to complete. Outside the zone the monitor sends `WAIT FOR THE ZONE` with a limit price, or `SETUP LATE` if the move is spent — neither resolves the watch. `false` restores entry at market. See `docs/confirmation-rules.md` |
| `ENTRY_LATE_TP1_FRACTION` | `0.3` | how much of the distance to TP1 price may already have travelled before the entry counts as late. The operator's own V14 rule. `0` disables it |
