...
if (promoted) {
  ...
  // The trap already said where its own thesis dies — the flip level.
  // That is a different statement from where a trade derived from it
  // would be stopped, and the promoted setup has to carry both or the
  // anti-SL branch has nothing to tell apart.
  assert(
    promoted.invalidation === 4360,
    `the trap's flip level became the setup's thesis line (${promoted.invalidation})`,
  );
  assert(
    promoted.invalidation !== promoted.sl,
    "which is not the same number as the stop",
  );
  assert(
    promoted.thesis_invalidation_declared === true,
    "and is marked as genuinely declared, so a stop touch opens the anti-SL branch instead of killing it",
  );

  // The whole point of promotion: what comes out the other side is an
  // ordinary setup on the ordinary state machine. It still has to be
  // touched, still has to prove its defence, still has to pass the gates.
  assert(
    promoted.defence_profile === "standard",
    `it proves the same defence a hand-registered setup proves (${promoted.defence_profile})`,
  );
  assert(
    ["REGISTERED_WATCH", "READY_FOR_ENTRY"].includes(promoted.stage),
    `and is waiting for its entry, not entered (${promoted.stage})`,
  );
  assert(promoted.entryTouched === false, "nothing has been entered by promoting it");
  assert(
    (promoted.trail || []).some((event) => event.type === "setup_created"),
    "and it starts its own event trail",
  );
}
