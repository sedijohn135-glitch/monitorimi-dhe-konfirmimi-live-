...
// ---------------------------------------------------------------------------
// §Q Entry quality — three reads the operator's own reference charts make,
// and the monitor could not.
//
// These answer questions the confirmation sequence does not ask. It proves
// the market *defended* the setup — rejection, structure shift,
// displacement, acceptance. These describe how *well* it did so: whether
// the push into the zone was running out of strength, whether three
// candles in a row pressed the same way, whether the entry sits in the
// retracement band the analysis says it should.
//
// They are REPORTED, never gates. A gate here would refuse entries the
// analysis meant, which is the failure mode that costs the most and shows
// the least — an entry that never fires looks exactly like a market that
// never came. The confirmation rules already decide; these tell the
// operator what he is about to take.

/**
 * Bodies shrinking into the zone, against the trade — the "Buyers Power"
 * read: each successive candle pressing the wrong way has less behind it
 * than the last.
 *
 * This is Model 9's "zbehje e volumit" and the tail of Model 11's
 * three-stage staging, measured on bodies rather than volume: tick volume
 * from a CFD feed describes the broker's flow, the body describes the
 * delivery, and only one of those is the market.
 */
export function momentumFade(bars, direction, { count = 3 } = {}) {
  if (!Array.isArray(bars) || bars.length < count) {
    return { present: false, known: false, reason: "not enough bars" };
  }
  const recent = bars.slice(-count);
  // Candles pressing against the trade: bearish ones for a buy.
  const against = direction === "buy" ? (bar) => !isBullish(bar) : (bar) => isBullish(bar);
  if (!recent.every(against)) {
    return { present: false, known: true, reason: "the push is not one-sided" };
  }
  const sizes = recent.map(bodySize);
  const shrinking = sizes.every((size, index) => index === 0 || size < sizes[index - 1]);
  return {
    present: shrinking,
    known: true,
    bodies: sizes,
    reason: shrinking
      ? `${count} counter-candles with shrinking bodies`
      : "counter-candles are not losing strength",
  };
}

/**
 * Three White Soldiers / Three Black Crows — three closes in a row in the
 * trade's direction, each body opening inside the last one's body and
 * closing beyond its close. The open-inside rule is what separates the
 * pattern from three unrelated candles that happen to share a colour.
 */
export function sequentialDrive(bars, direction, { count = 3 } = {}) {
  if (!Array.isArray(bars) || bars.length < count) {
    return { present: false, known: false, reason: "not enough bars" };
  }
  const recent = bars.slice(-count);
  const wanted = direction === "buy";
  for (let index = 0; index < recent.length; index += 1) {
    const bar = recent[index];
    if (isBullish(bar) !== wanted) {
      return { present: false, known: true, reason: "the run is broken" };
    }
    if (index === 0) continue;
    const previous = recent[index - 1];
    const opensInside = wanted
      ? bar.open > bodyBottom(previous) && bar.open < bodyTop(previous)
      : bar.open < bodyTop(previous) && bar.open > bodyBottom(previous);
    const closesBeyond = wanted ? bar.close > previous.close : bar.close < previous.close;
    if (!opensInside || !closesBeyond) {
      return { present: false, known: true, reason: "not a stepped sequence" };
    }
  }
  return {
    present: true,
    known: true,
    reason: direction === "buy" ? "three white soldiers" : "three black crows",
  };
}

/**
 * The OTE band — 0.618 to 0.79 of the leg, measured from its origin, which
 * is where Models 9 (OTE) and 35 (FBE) say the entry belongs.
 *
 * The leg is taken from the swings the monitor already computes rather
 * than from anything the analysis asserts, so this is an independent read
 * of the same chart: agreement means two derivations landed on the same
 * band, and disagreement is worth seeing before committing.
 */
export function oteZone(bars, direction, { lookback = 60 } = {}) {
  const window = Array.isArray(bars) ? bars.slice(-lookback) : [];
  const swings = strictSwings(window);
  const highs = swings.filter((swing) => swing.kind === "high");
  const lows = swings.filter((swing) => swing.kind === "low");
  if (!highs.length || !lows.length) {
    return { known: false, reason: "no swing leg to measure" };
  }
  const high = highs.reduce((best, swing) => (swing.price > best.price ? swing : best));
  const low = lows.reduce((best, swing) => (swing.price < best.price ? swing : best));
  const span = high.price - low.price;
  if (!(span > 0)) return { known: false, reason: "degenerate leg" };

  // A buy retraces down from the high; a sell retraces up from the low.
  const zone =
    direction === "buy"
      ? { low: high.price - span * 0.79, high: high.price - span * 0.618 }
      : { low: low.price + span * 0.618, high: low.price + span * 0.79 };
  return {
    known: true,
    low: Math.min(zone.low, zone.high),
    high: Math.max(zone.low, zone.high),
    leg: { high: high.price, low: low.price },
  };
}

/** Whether a price sits inside that band. Unknown when the leg cannot be read. */
export function inOteZone(bars, price, direction, options = {}) {
  const zone = oteZone(bars, direction, options);
  if (!zone.known || !Number.isFinite(price)) {
    return { present: false, known: false, reason: zone.reason || "no price" };
  }
  ...
  return {
    ...
    reason: inside
      ? `inside the 0.618–0.79 band (${zone.low.toFixed(2)}–${zone.high.toFixed(2)})`
      : `outside the 0.618–0.79 band (${zone.low.toFixed(2)}–${zone.high.toFixed(2)})`,
  };
}
