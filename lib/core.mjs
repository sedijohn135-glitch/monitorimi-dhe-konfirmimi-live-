...
export function evaluateConfirmation(watch, signals, ctx) {
  ...
  // The standard lane, unchanged: acceptance plus at least one graduated
  // technical signal. It is the stronger proof and so it is tried first —
  // when both lanes qualify, this is the one that gets reported.
  if (acceptance && technical.length > 0) {
    return { evidence, enter: true, signals: labels, strength: strengthOf(), lane: "standard" };
  }

  // §19 — the CISD fast lane.
  //
  // Acceptance asks price to travel a buffer beyond the entry before the
  // level counts as accepted. That is a sound proof and a structurally
  // late one: it cannot be satisfied until the move has already happened,
  // so the earliest signal in the set (CISD needs two bars — a sweep and
  // a close that reclaims it) ends up waiting on the latest, and the fill
  // arrives well past the price the analysis named.
  //
  ...
  // "structure broke and price is still standing on the level" — CISD
  // graduated, a strong rejection printed with it, and price has not left
  // the entry zone. That last condition is what makes the substitution
  // safe rather than merely faster: slippage is bounded by the width of
  // the zone by construction, so the fill cannot be a different trade
  // from the planned one, and invalidation sits close enough that a wrong
  // read is discovered immediately. Every downstream gate still applies —
  // kill zone, spread, news, and the R:R floor at the fill.
  //
  // Nothing is removed. This is an additional route to `enter`, and a
  // setup that fails it still has the standard lane waiting behind it.
  const fastLaneReady =
    ctx.cisdFastLane === true &&
    ctx.inEntryZone === true &&
    evidence.structure.graduated === true &&
    (ctx.cisdFastLaneRequireStrongPattern === false ||
      (evidence.pricePattern.graduated === true && evidence.pricePattern.tier === "strong"));
  if (fastLaneReady) {
    return {
      evidence,
      enter: true,
      signals: labels.concat(["In-Zone (no acceptance required)"]),
      strength: strengthOf(),
      lane: "cisd_fast",
    };
  }
...
export function validateWatchInput(args) {
  ...
  // Which target the R:R floor answers to. Rejected rather than defaulted
  // when it names a target the setup did not send, because a silent
  // fallback here is a setup judged on a different trade than the analyst
  // meant — the exact failure this field exists to prevent.
  const rrTarget = args.rr_target === undefined || args.rr_target === null || args.rr_target === ""
    ? null
    : String(args.rr_target).toLowerCase();
  if (rrTarget !== null) {
    if (!RR_TARGET_KEYS.includes(rrTarget)) {
      throw new Error(`rr_target must be one of ${RR_TARGET_KEYS.join(", ")}`);
    }
    if (finiteNumber(numeric[rrTarget]) === null) {
      throw new Error(`rr_target is ${rrTarget}, but no ${rrTarget} was sent`);
    }
  }
...
/**
 * §19/§24 — the same hold, shortened while price is still standing in the
 * entry zone.
 *
 * A long hold buys protection against a signal that graduates and then
 * fades, and it is worth paying for once price has left the level: the
 * evidence is all that is left to trust, so make it prove itself. Inside
 * the zone the trade-off inverts. Waiting there costs the thing the hold
 * was protecting — every second spent holding is a second price can spend
 * walking away from the entry, and a confirmation that arrives after the
 * walk is a worse trade at a worse ratio. Invalidation is also close at
 * hand while price is in the zone, so a false read is cheap to discover
 * and cheap to survive.
 *
 * The hold is never removed, only lowered to the floor every other path
 * is already bounded by, and the caller keeps its new-bar requirement.
 */
export function proximityHoldMs(watch, mid, tolerance, holdMs, floorMs = SKILL_CONTEXT_LIMITS.holdFloorMs) {
  const hold = finiteNumber(holdMs) ?? 60_000;
  const floor = finiteNumber(floorMs) ?? SKILL_CONTEXT_LIMITS.holdFloorMs;
  if (!priceInEntryZone(watch, mid, tolerance)) return hold;
  return Math.min(hold, Math.max(floor, 0));
}

/**
 * Whether price is standing inside the band the setup called its entry.
 * Deliberately inclusive of both edges: the edge is the level the analyst
 * named, and a fill there is the fill that was planned.
 */
export function priceInEntryZone(watch, mid, tolerance) {
  const price = finiteNumber(mid);
  if (price === null) return false;
  const zone = entryZone(watch, tolerance);
  const low = finiteNumber(zone.low);
  const high = finiteNumber(zone.high);
 ...
 *
 * Confirmation validates the SETUP. It does not license an entry at
 * whatever price the market happens to be at when the last signal
 * graduates — and treating it as if it did is what put the operator into
 * a live short 5.59 below the analysed entry, on 34% more risk than the
 * setup was designed to carry, moments before price ran to within 1.63 of
 * the stop. The analysis was right about the trade the whole time. The
 * price it was taken at was not the one that was analysed.
 *
 * Three outcomes, and only the first is an entry:
 *
 *   ENTER_NOW      price is in the zone (or has drifted in the operator's
 *                  favour) — this is the analysed trade, take it
 *   SETUP_LATE     price has already delivered `lateTp1Fraction` of the way
 *                  to TP1 — the move is spent and what is left to collect
 *                  is the retracement
 *   WAIT_FOR_ZONE  the setup is alive but this is not its price; work a
 *                  limit at the analysed entry instead
 *
 * Neither of the last two resolves the watch. That distinction is the
 * whole point: the operator has been burned by a monitor that called a
 * live setup finished, and by one that called a bad price an entry. "Not
 * at this price" is a third answer, and it is the honest one — six
 * minutes after the refusal above, price traded back through the zone.
 */
export function entryPlacement(
  watch,
  { mid, tolerance = 0, lateTp1Fraction = 0.3, zoneOnly = true, graceRiskFraction = 0.1, roomRiskFraction = 0.5 } = {},
) {
  const price = finiteNumber(mid);
  const shape = { limitPrice: watch.entry, progressToTp1: null, driftFromEntry: null };
  if (price === null) return { ...shape, action: "WAIT_FOR_ZONE", resolves: false };

  if (!zoneOnly) return { ...shape, action: "ENTER_NOW", resolves: false };

  // Only drift that makes the fill worse counts. A sell filling above its
  // zone is a better trade than the one that was analysed, not a worse one.
  const isBuy = watch.direction === "buy";
  const drift = isBuy ? price - watch.entry : watch.entry - price;
  // The zone edge is not a laser line — spread alone is 0.6 on gold — and
  // a drift too small to move the ratio is not a different trade. The
  // grace is a fraction of the setup's own risk rather than a fixed
  // distance, because the same absolute drift costs a 50-point stop a
  // fifth of what it costs a 5-point one. Drift moves R:R roughly
  // one-for-one: the live failure drifted 34% of risk and turned 2.88R
  // into 1.90R, also 34%.
  //
  // Measured from the ZONE, not from the entry: the analyst declared the
  // whole band acceptable, so a fill anywhere inside it is the analysed
  // trade by construction and only what lies beyond it is drift.
  const risk = Math.abs(watch.entry - watch.sl);
  const grace = Math.max(finiteNumber(tolerance) ?? 0, risk * graceRiskFraction);
  const zone = entryZone(watch, tolerance);
  const excess = isBuy ? price - Math.max(zone.low, zone.high) : Math.min(zone.low, zone.high) - price;
  if (priceInEntryZone(watch, price, tolerance)) {
    return { ...shape, action: "ENTER_NOW", resolves: false, driftFromEntry: drift };
  }
  // Price the operator's side of the zone is a better price only while the
  // stop is still a stop. Selling 14.87 above the plan reads as a gift
  // until you notice it leaves 1.63 of room on a setup built to risk
  // 16.50 — a different trade, one that noise closes, taken at the
  // analyst's own invalidation. So it waits for the zone like any other.
  if (drift < 0) {
    const roomLeft = isBuy ? price - watch.sl : watch.sl - price;
    const action = risk <= 0 || roomLeft >= risk * roomRiskFraction ? "ENTER_NOW" : "WAIT_FOR_ZONE";
    return { ...shape, action, resolves: false, driftFromEntry: drift };
  }
  if (excess <= grace) {
    return { ...shape, action: "ENTER_NOW", resolves: false, driftFromEntry: drift };
  }

  const toTp1 = Math.abs(watch.entry - finiteNumber(watch.tp1));
  const progressToTp1 = toTp1 > 0 ? drift / toTp1 : 0;
  if (lateTp1Fraction > 0 && progressToTp1 >= lateTp1Fraction) {
    return { ...shape, action: "SETUP_LATE", resolves: false, progressToTp1, driftFromEntry: drift };
  }
  return { ...shape, action: "WAIT_FOR_ZONE", resolves: false, progressToTp1, driftFromEntry: drift };
}

export const RR_TARGET_KEYS = Object.freeze(["tp1", "tp2", "tp3"]);

/**
 * Which target the R:R floor is measured to, resolved to a target the
 * watch actually carries.
 *
 * An override argument wins, then the setup's own `rr_target`, then TP1.
 * A declared target that is missing from the watch falls back rather than
 * throwing: a setup naming TP2 as its objective and then not sending one
 * is an analysis mistake, and refusing to measure at all would turn that
 * into a silent no-entry.
 */
export function rrTargetKey(watch, override = null) {
  for (const candidate of [override, watch?.rr_target]) {
    const key = String(candidate || "").toLowerCase();
    if (RR_TARGET_KEYS.includes(key) && finiteNumber(watch?.[key]) !== null) return key;
  }
  return "tp1";
}
...
export function evaluateEntryOpportunity(
  watch,
  {
    ...
    riskFraction = 0.3,
    minRemainingRR = 0.5,
    // "tp1" | "tp2" | "tp3", or null to use the setup's own declaration.
    rrTarget = null,
  ...
  //
  // TP1 is not always the trade. An analysis that names a Draw On
  // Liquidity is describing a delivery *to the DOL*, and the nearer
  // targets are partials taken on the way — one of them is literally
  // labelled "Low Hanging Fruit" in the template this monitor is fed.
  // Judging such a setup on TP1 alone answers a question nobody asked:
  // a fill still worth 3.4R to the objective was refused for being 1.2R
  // to the fruit, on a trade that then delivered. The objective is the
  // analyst's to declare — `rr_target` carries it — and TP1 stays the
  // default, so a setup that declares nothing behaves exactly as before.
  const targetKey = rrTargetKey(watch, rrTarget);
  const target = finiteNumber(watch[targetKey]);
  const rewardTarget = target === null ? watch.tp1 : target;
  const rewardTargetName = target === null ? "TP1" : targetKey.toUpperCase();
  ...
  // The ratio the analysis promised, measured from the planned entry to
  // the same target. The gate below decides on `remainingRR` alone —
  // what the operator would actually get — but the notification has to be
  // able to say what the promise was, because the whole point of standing
  // a setup down is that the two numbers no longer match.
  const plannedReward = isBuy ? rewardTarget - watch.entry : watch.entry - rewardTarget;
  const rrPlanned = risk > 0 ? plannedReward / risk : null;
  ...
  const shape = {
    ...
    rrPlanned,
    riskPlanned: risk,
    rrTarget: rewardTargetName,
    rrTargetLevel: rewardTarget,
  };
  ...
  if (remainingRR !== null && remainingRR < minRemainingRR) {
    return {
      ...
      reason: "RR_COLLAPSED",
      detail:
        `only ${remainingRR.toFixed(2)}R remains to ${rewardTargetName} from here` +
        (rrPlanned !== null ? ` against ${rrPlanned.toFixed(2)}R planned` : "") +
        `; ${minRemainingRR}R is the floor` +
        (chase > 0 ? ` (fill is ${formatLevel(chase)} beyond the planned entry` : "") +
        (chase > 0 && remainingRisk > 0 ? `, risk ${formatLevel(remainingRisk)} against ${formatLevel(risk)} planned)` : chase > 0 ? ")" : ""),
    };
  }
...
/**
 * Does a refused entry end the setup, or only this moment of it?
 *
 * Only one refusal is terminal. RISK_INVERTED means price is at or through
 * the stop: there is no trade left to take at any price, and no retrace
 * brings one back.
 *
 * The others describe where price is standing, not what the setup is worth.
 * RR_COLLAPSED in particular is arithmetic on the live price — the analysed
 * entry, stop and targets are untouched, and a retrace into the zone
 * restores the ratio exactly as it was analysed. Resolving the watch there
 * threw away a live setup and told the operator to re-run an analysis that
 * had not changed.
 *
 * What ends a setup is the stop, the thesis invalidation, or the entry
 * window closing. Each has its own clock. The ratio is not one of them.
 */
export function opportunityEndsSetup(reason) {
  return reason === "RISK_INVERTED";
}
