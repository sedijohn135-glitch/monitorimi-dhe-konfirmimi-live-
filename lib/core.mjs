...
export function evaluateEntryOpportunity(
  ...
  // The ratio the analysis promised, measured from the planned entry. The
  // gate below decides on `remainingRR` alone — what the operator would
  // actually get — but the notification has to be able to say what the
  // promise was, because the whole point of standing a setup down is that
  // the two numbers no longer match.
  const plannedReward = isBuy ? watch.tp1 - watch.entry : watch.entry - watch.tp1;
  ...
  if (remainingRR !== null && remainingRR < minRemainingRR) {
    return {
      ...
      reason: "RR_COLLAPSED",
      detail:
        `only ${remainingRR.toFixed(2)}R remains to TP1 from here` +
