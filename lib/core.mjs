...
export function stageOf(watch) {
  ...
  if (watch.entryTouched) {
    ...
    // A watch on the skill-context fast lane, or one the analyst
    // registered on the M1 defence profile, may never print the M5
    // sequence — its proof obligation was met on M1 — so reporting it as
    // ENTRY_TOUCHED would hide the only thing it has actually proven.
    if (lane === "M1_FAST" || lane === "SKILL_VALIDATED") return "M1_CONTINUATION_CONFIRMED";
...
// ---------------------------------------------------------------------------
// §11b Skill context — what the analyst proved before the touch
//
// The analysis skill knows things this monitor cannot see: the HTF
// market-structure shift it read 30 minutes ago, the trap phase it
// classified, the liquidity it watched get swept, and — critically —
// whether the M5 structure shift already printed *before* price returned
// to the entry zone. Without that context the monitor re-derives the
// same sequence from scratch after the touch and enters five to fifteen
// minutes behind the move the analyst actually called.
//
// So the skill may now hand its own conclusions to `register_watch`, and
// the monitor may use them to shorten the confirmation path. Three rules
// bound what that is allowed to mean, and every function here exists to
// enforce one of them:
//
//   1. A claim is never evidence. The skill's context can decide *which*
//      live proof is required and *how long* it must hold — it can never
//      stand in for proof. The fast lane still demands market-structure
//      shift and displacement on closed bars that closed after the
//      touch; it demands them on M1 instead of M5.
//   2. Nothing here touches safety. Stop loss, invalidation, expiry,
//      spread, news, kill zone and the price-scale falsification run
//      exactly as they did before, in every lane.
//   3. Claims expire and are audited. A context older than its declared
//      freshness window stops conferring anything, a context that
//      disagrees with the setup it is attached to confers nothing at
//      all, and every claim is recorded against what the market then did.
...
/**
 * The conditions the skill may attach to `skip_m5_sequence_if` and
 * `require_m1_only_if`. They name a flag in the same context, so the
 * skill declares "shorten the path, but only if the claim I made is the
 * one that justifies shortening it" rather than an unconditional
 * request. `never` is the default: an absent condition changes nothing.
 */
export const SKILL_CONTEXT_CONDITIONS = Object.freeze([
  "never",
  "always",
...
export const SKILL_CONTEXT_LIMITS = Object.freeze({
  // The shortest confirmation hold any context can buy. A hold below
  // this is not a fast confirmation, it is no confirmation.
  holdFloorMs: 15_000,
  maxHoldMs: 15 * 60_000,
  defaultMaxAgeMs: 180_000,
  // A skill cannot declare its own analysis fresh forever.
  maxAgeCapMs: 15 * 60_000,
...
function conditionMet(condition, context) {
  switch (condition) {
    case "always":
      return true;
    case "m5_mss_already_observed":
      return context.m5_mss_already_observed === true;
    case "htf_mss_confirmed":
      return context.htf_mss_confirmed === true;
    case "liquidity_swept":
      return context.liquidity_swept === true;
    default:
      return false;
  }
}
...
export function validateSkillContext(
  ...
  const condition = (key) => {
    const value = String(source[key] ?? "never").trim().toLowerCase();
    if (!SKILL_CONTEXT_CONDITIONS.includes(value)) {
      warnings.push(`${key}="${value}" is not a known condition and is treated as never`);
      return "never";
    }
    return value;
  };
  ...
  let suggestedHold = number("suggested_min_hold_ms");
  if (suggestedHold !== null) {
    if (suggestedHold < 0) {
      warnings.push("suggested_min_hold_ms cannot be negative and was dropped");
      suggestedHold = null;
    } else if (suggestedHold > limits.maxHoldMs) {
      warnings.push(`suggested_min_hold_ms was clamped to ${limits.maxHoldMs}ms`);
      suggestedHold = limits.maxHoldMs;
    }
  }

  let maxAge = number("suggested_max_age_ms");
  if (maxAge === null) {
    maxAge = limits.defaultMaxAgeMs;
  } else if (!(maxAge > 0)) {
    warnings.push(`suggested_max_age_ms must be greater than zero; using ${limits.defaultMaxAgeMs}ms`);
    maxAge = limits.defaultMaxAgeMs;
  } else if (maxAge > limits.maxAgeCapMs) {
    warnings.push(`suggested_max_age_ms was clamped to ${limits.maxAgeCapMs}ms`);
    maxAge = limits.maxAgeCapMs;
  }
  ...
  const context = {
    ...
    htf_bias: htfBias,
    conviction,
    suggested_min_hold_ms: suggestedHold,
    suggested_max_age_ms: maxAge,
    skip_m5_sequence_if: condition("skip_m5_sequence_if"),
    require_m1_only_if: condition("require_m1_only_if"),
  ...
  // A claim that contradicts the setup it is attached to is not a
  // shortcut, it is a reason to stop. The watch still registers — a
  // counter-trend setup is a legitimate thing to trade — but it earns
  // no fast lane.
  context.direction_conflict =
    direction !== null && context.htf_bias !== null && context.htf_bias !== direction;
  if (context.direction_conflict) {
    warnings.push(
      `htf_bias=${context.htf_bias} disagrees with a ${direction} setup; the fast lane is refused`,
    );
  }
  // "Skip the M5 sequence because M5 MSS already printed" is only
  // coherent if the skill actually claimed the M5 MSS printed.
  for (const key of ["skip_m5_sequence_if", "require_m1_only_if"]) {
    if (context[key] !== "never" && !conditionMet(context[key], context)) {
      warnings.push(`${key}="${context[key]}" is declared but its claim is not set`);
    }
  }
...
/**
 * The fast lane's proof obligation. It is the M5 sequence's obligation
 * on a faster clock, not a smaller one: a market-structure shift and a
 * displacement, both carried by bars that closed *after* the touch. A
 * skill claiming the M5 shift already printed buys the right to prove
 * continuation on M1 — it does not buy the right to skip proving it.
 */
...
/**
 * Turn the skill's context and this tick's observations into the two
 * decisions the monitor actually needs: how long evidence must hold, and
 * whether the full M5 sequence is still required.
 *
 * Pure, and deliberately verbose in what it returns — `reasons` and
 * `blocked` are the audit trail for why a trade was taken early or why a
 * HIGH-conviction context bought nothing at all.
 */
export function applySkillContext(context, observations = {}, config = {}) {
  const baseHoldMs = finiteNumber(config.minHoldMs) ?? 60_000;
  const holdFloorMs = finiteNumber(config.holdFloorMs) ?? SKILL_CONTEXT_LIMITS.holdFloorMs;
  const sequenceRequired = config.sequenceRequired !== false;
  const forwardState = observations.forwardState || "WITHIN_ZONE";
  const decision = {
    lane: "STANDARD",
    holdMs: baseHoldMs,
    requireSequence: sequenceRequired,
    forwardState,
    eligible: false,
    reasons: [],
    blocked: [],
  ...
  if (!context || config.enabled === false) {
    if (context && config.enabled === false) decision.blocked.push("skill context is disabled by configuration");
    return decision;
  }

  const conviction = context.conviction || "MEDIUM";
  const adverse = forwardState === "FAILED" || forwardState === "STALE";

  // -- The hold ------------------------------------------------------------
  // Lengthening is always allowed: a skill asking for more proof gets
  // more proof. Shortening is a privilege, and it is priced accordingly.
  const suggested = finiteNumber(context.suggested_min_hold_ms);
  let lengthened = false;
  if (suggested !== null) {
    if (suggested >= baseHoldMs) {
      decision.holdMs = suggested;
      lengthened = suggested > baseHoldMs;
      decision.reasons.push(`skill lengthened the confirmation hold to ${suggested}ms`);
    } else if (adverse) {
      decision.blocked.push(`hold shortening refused: forward validation is ${forwardState}`);
    } else if (conviction !== "HIGH") {
      decision.blocked.push(`hold shortening refused: conviction is ${conviction}, not HIGH`);
    } else if (context.direction_conflict) {
      decision.blocked.push("hold shortening refused: htf_bias disagrees with the setup direction");
    } else {
      decision.holdMs = Math.max(suggested, holdFloorMs);
      decision.reasons.push(
        `skill shortened the confirmation hold to ${decision.holdMs}ms on HIGH conviction`,
      );
    }
  }

  // -- The lane ------------------------------------------------------------
  const wantsSkip = conditionMet(context.skip_m5_sequence_if, context);
  const wantsM1Only = conditionMet(context.require_m1_only_if, context);
  if (!wantsSkip && !wantsM1Only) {
    decision.reasons.push("skill did not ask for a shortened sequence");
    return decision;
  }
  if (config.fastLaneEnabled === false) {
    decision.blocked.push("fast lane disabled by configuration");
    return decision;
  }
  if (context.direction_conflict) {
    decision.blocked.push("fast lane refused: htf_bias disagrees with the setup direction");
    return decision;
  }
  if (forwardState === "FAILED") {
    decision.blocked.push("fast lane refused: price moved against the setup after the touch");
    return decision;
  }
  if (forwardState === "STALE") {
    decision.blocked.push("fast lane refused: the skill's context is past its freshness window");
    return decision;
  }
  if (forwardState === "WITHIN_ZONE") {
    decision.blocked.push("fast lane pending: price has not moved out of the zone in either direction");
    return decision;
  }

  decision.eligible = true;
  const m1 = observations.m1Continuation || null;
  if (!m1 || m1.ok !== true) {
    decision.blocked.push(
      `fast lane pending: ${m1?.reason || "M1 continuation has not been verified"}`,
    );
    return decision;
  }

  decision.requireSequence = false;
  if (forwardState === "STRONG_MOVE" && conviction === "HIGH") {
    decision.lane = "SKILL_VALIDATED";
    decision.reasons.push(
      "HIGH conviction plus a strong post-touch move, verified by M1 structure shift and displacement",
    );
    // A skill that explicitly asked for a longer hold keeps it. Two
    // instructions that disagree resolve toward more proof, never less.
    if (!lengthened && decision.holdMs > holdFloorMs) {
      decision.holdMs = holdFloorMs;
      decision.reasons.push(`confirmation hold reduced to the ${holdFloorMs}ms floor`);
    }
  } else {
    decision.lane = "M1_FAST";
    decision.reasons.push(
      `${forwardState} with M1 structure shift and displacement since the touch replaces the M5 sequence`,
    );
  }
  return decision;
}
...
export function skillContextAudit(watch, status, { nowMs = Date.now() } = {}) {
  ...
  const progressR = finiteNumber(state.forward?.progressR);
  const m1 = state.m1Continuation || null;
  return {
    verdict: `skill_said_${context.conviction}_${confirmed ? "and" : "but"}_${outcome}`,
    conviction: context.conviction,
    outcome: status,
    lane: state.lane || "STANDARD",
    forward_state: state.forwardState || null,
    claimed: {
      ...
      suggested_min_hold_ms: context.suggested_min_hold_ms,
      suggested_max_age_ms: context.suggested_max_age_ms,
      skip_m5_sequence_if: context.skip_m5_sequence_if,
      require_m1_only_if: context.require_m1_only_if,
      direction_conflict: context.direction_conflict === true,
      warnings: context.warnings?.length ? context.warnings : null,
    },
    observed: {
      hold_ms_used: finiteNumber(state.holdMs),
      base_hold_ms: finiteNumber(state.baseHoldMs),
      progress_r: progressR === null ? null : Number(progressR.toFixed(3)),
      m5_sequence_complete: watch.sequence?.complete === true,
      m1_continuation: m1
        ? {
            ok: m1.ok === true,
            bars_after_touch: m1.barsAfterTouch ?? 0,
            mss: m1.mss?.present === true,
            displacement: m1.displacement?.present === true,
          }
        : null,
      fast_lane_blocked: state.blocked?.length ? state.blocked : null,
      reasons: state.reasons?.length ? state.reasons : null,
      touch_to_confirm_ms:
...
export function summariseSkillContextAudits(audits) {
  ...
  for (const audit of list) {
    ...
    countInto(byLane, audit.lane || "STANDARD", audit.outcome);
    countInto(byForwardState, audit.forward_state, audit.outcome);
    const hold = finiteNumber(audit.observed?.hold_ms_used);
    if (hold !== null) {
      holdSum += hold;
      holdCount += 1;
    }
  ...
  return {
    ...
    by_lane: byLane,
    by_forward_state: byForwardState,
    mean_hold_ms: holdCount ? Math.round(holdSum / holdCount) : null,
    ...
    note:
      ...
      "LOW conviction that keeps resolving CONFIRMED is a conservative one. Compare by_lane before " +
      "widening the fast lane: it is only worth having if its outcomes match the standard sequence's.",
  };
}
...
export function evaluateEntryOpportunity(
  ...
  const remainingReward = isBuy ? watch.tp1 - price : price - watch.tp1;
  ...
  if (remainingRR !== null && remainingRR < minRemainingRR) {
    return {
      ...
      actionable: false,
      reason: "RR_COLLAPSED",
      detail: `only ${remainingRR.toFixed(2)}R remains to TP1 from here; ${minRemainingRR}R is the floor`,
    };
  }
...
export function evaluateTimeWindow(
  watch,
  { nowMs = Date.now(), killZoneActive = null, killZoneEnabled = true } = {},
) {
  const deadline = finiteNumber(watch.confirmationDeadlineAt);
  ...
  if (deadline !== null && nowMs >= deadline) {
    return {
      ...
      detail: `the confirmation deadline passed ${Math.round((nowMs - deadline) / 1000)}s ago`,
      deadline,
      expiry,
    };
  }
