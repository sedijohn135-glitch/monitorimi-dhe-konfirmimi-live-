...
export function publicWatch(watch) {
  ...
  // actually proven.
  //
  // `degraded` is the structural-erosion flag hoisted to the top level.
  // The count and the defensiveExit boolean were already here inside
  // `structureFailure`, but a reader scanning a list of open trades for
  // the one going wrong had to go looking for them; the position whose
  // read has already failed is exactly the one that should not need
  // looking for. It reports erosion, not a stop — the stop still applies
  // and nothing is closed on the strength of this field.
  return {
    ...rest,
    stage: stageOf(watch),
    degraded: watch.structureFailure?.defensiveExit === true,
  };
}
