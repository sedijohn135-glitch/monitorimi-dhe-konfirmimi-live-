 ...
 */
/**
 * What a fired time stop should ask for, given where price is.
 *
 * Time Distortion says the expansion did not arrive in the bars the model
 * allows. It does not say the thesis is wrong — and price above a buy's
 * entry is the market's own evidence that it is not. Closing there books
 * a small win against the setup's own objective, so in profit the answer
 * is break-even: the risk goes to zero and the position stays alive.
 *
 * At or below the entry nothing has been earned and nothing is protected.
 * That trade is dead weight and the answer is still CLOSE.
 *
 * Only TIME_STOP asks this. The other three switches fire on structure —
 * a breaker taken, an HTF cascade, three PDA arrays broken — and each of
 * those says the thesis itself has failed. Being green does not soften
 * them, so they are never routed through here.
 */
export function timeStopResponse(position, price) {
  const now = finiteNumber(price);
  const entry = finiteNumber(position?.entry);
  if (now === null || entry === null) return "CLOSE";
  const profitable = position?.direction === "buy" ? now > entry : now < entry;
  return profitable ? "BREAK_EVEN" : "CLOSE";
}
