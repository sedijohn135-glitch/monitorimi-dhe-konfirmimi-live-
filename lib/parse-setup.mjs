...
const LABELS = {
  symbol:          ["INSTRUMENT", "SYMBOL", "PAIR", "ASSET", "MARKET"],
  direction:       ["DIRECTION", "SIDE", "BIAS", "TYPE"],
  entry:           ["SNIPER 0 FLOAT ENTRY PRICE", "SNIPER ENTRY", "FINAL ENTRY", "ENTRY PRICE", "ENTRY"],
  entry_zone:      ["ENTRY ZONE", "ZONA HYRJE", "ZONA ENTRY", "TP ENTRY", "TP_ENTRY"],
  sl:              ["SL ZONE", "STOP LOSS", "STOPLOSS", "STOP", "SL"],
  invalidation:    ["INVALIDATION", "THESIS INVALIDATION", "INVAL LEVEL"],
  tp1:             ["TARGET 1", "TP 1", "TP1", "T1", "RR1"],
  tp2:             ["TARGET 2", "TP 2", "TP2", "T2", "RR2"],
  tp3:             ["TARGET 3", "TP 3", "TP3", "T3", "RR3"],
  setup_model:     ["ENTRY MODEL", "MODEL", "STRATEGY", "SETUP TYPE"],
...
export function parseSetupText(text) {
  ...
  if (out.direction === null) {
    ...
    if (/\bSHORT\b/.test(text) && !/\bLONG\b/.test(text)) {
      out.direction = "sell";
      recognised.add("direction");
      warnings.push("direction inferred as SELL from standalone SHORT");
    } else if (/\bLONG\b/.test(text) && !/\bSHORT\b/.test(text)) {
      out.direction = "buy";
      recognised.add("direction");
      warnings.push("direction inferred as BUY from standalone LONG");
