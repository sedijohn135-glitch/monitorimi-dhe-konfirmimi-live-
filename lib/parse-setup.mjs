...
const LABELS = {
  ...
  trap_zone:       [
    ...
    // The PDA band. In the v6 template the entry is a single price inside
    // it and the trigger is declared as firing *within* the band, so the
    // band — not the price — is the zone the monitor should watch. Without
    // it the zone falls back to entry ± tolerance, which on gold is 0.20
    // wide: too narrow for any in-zone rule to be reachable, so the only
    // confirmation left is the one that waits for price to travel away.
    "PDA", "PDA ZONE", "PDA ARRAY", "ZONA PDA", "OB ZONE", "FVG ZONE",
  ],
  // The Draw On Liquidity — the level the whole delivery is aimed at.
  // Matched against the declared targets to decide which one the R:R
  // floor answers to; see §dol below.
  dol:             ["DOL", "DRAW ON LIQUIDITY", "PRIMARY TARGET", "TARGETI PRIMAR", "OBJEKTIVI"],
  rr:              ["RR", "R:R", "RRR", "RISK REWARD", "RISK:REWARD"],
  sl:              ["STOP HUMBJES", "SL ZONE", "STOP LOSS", "STOPLOSS", "STOP", "SL"],
  // "INVALID" last: the alternation is tried in order, so the longer
  // spellings must get first refusal on a line that starts with them.
  invalidation:    ["INVALIDATION", "THESIS INVALIDATION", "INVAL LEVEL", "INVALID"],
...
/**
 * Which block a zone line came from, for the warning that reports it.
 * The operator reads that line in Telegram to answer "where did this
 * zone come from", and every zone label naming itself KURTHI made the
 * answer wrong for every other source.
 */
function zoneLabelOf(line) {
  const head = String(line).trim().slice(0, 40).toUpperCase();
  if (head.startsWith("PDA")) return "PDA array";
  if (head.includes("OB ZONE") || head.includes("FVG ZONE")) return "declared array zone";
  if (head.includes("SNIPER")) return "sniper zone";
  return "KURTHI landing zone";
}
...
function matchLabel(line, labels) {
  ...
  // A label may carry a parenthetical qualifier before its separator —
  // "Fibo Master Sniper Zone (5.0–16.8): …", "TARGET 1 (M15 EQL): …".
  // Without tolerating it the line reads as narrative and the value is
  // lost.
  const re = new RegExp(
    `^[\\s\\u00A0]*(?:${escaped})(?:\\s*\\([^)]{0,60}\\))?(?:\\s*[:=\\-–—]\\s*|\\s{2,})(.+?)\\s*$`,
    "i",
  );
...
export function parseSetupText(text) {
  ...
  // Captured but not published. The analysis often names several zones —
  // a landing zone, a Fibo sniper zone — and only some of them belong to
  // the leg being traded, so they are all collected and the consistency
  // check below picks the one that fits.
  const trapZones = [];
  // A "zone" label carrying a single number is a level, not a zone. Held
  // here and used as the entry when no explicit entry price is given.
  let zoneLevel = null;
  // The declared Draw On Liquidity, held until the targets are known.
  let dolLevel = null;
  // The analysis's own reward-to-risk ("RR: 1:2.8"), held for the same
  // reason: it names an objective, and which target that is can only be
  // worked out once the levels are in.
  let declaredRr = null;
  ...
  for (const rawLine of lines) {
    ...
    if (out.entry_zone_low === null && zoneLevel === null) {
      const v = matchLabel(line, LABELS.entry_zone);
      if (v) {
        const [low, high] = parseRange(v);
        if (low !== undefined && high !== undefined && low !== high) {
          out.entry_zone_low = Math.min(low, high);
          out.entry_zone_high = Math.max(low, high);
          recognised.add("entry_zone_low");
          recognised.add("entry_zone_high");
          continue;
        }
        // A single number under a zone label is a LEVEL, not a zone —
        // the V13 table writes it that way ("ENTRY ZONE : [Level]").
        // Recording it as low === high made every such setup fail
        // validation, and widening it would invent a tolerance the
        // analyst never stated. It becomes the entry instead.
        const single = toNumber(v);
        if (single !== null) { zoneLevel = single; continue; }
      }
    }

    if (dolLevel === null) {
      const rr = matchLabel(line, LABELS.rr);
      if (rr !== null && declaredRr === null) {
        // "1:2.8", "2.8R", "2.8" — take the part after the colon when the
        // ratio is written as 1:X, the number itself otherwise.
        const ratio = /(?:^|\D)1\s*[:/]\s*(\d+(?:[.,]\d+)?)/.exec(rr) || /(\d+(?:[.,]\d+)?)\s*r\b/i.exec(rr);
        const value = ratio ? Number(String(ratio[1]).replace(",", ".")) : null;
        if (value !== null && Number.isFinite(value) && value > 0) declaredRr = value;
      }

      const v = matchLabel(line, LABELS.dol);
      if (v) {
        const level = toNumber(v);
        if (level !== null) {
          dolLevel = level;
          continue;
        }
    ...
    if (trapZones.length < 8) {
      const v = matchLabel(line, LABELS.trap_zone);
      if (v) {
        const [low, high] = parseRange(v);
        if (low !== undefined && high !== undefined && low !== high) {
          trapZones.push({ low: Math.min(low, high), high: Math.max(low, high), label: zoneLabelOf(line) });
          continue;
        }
      }
    }
  ...
  if (out.entry === null && zoneLevel !== null) {
    out.entry = zoneLevel;
    recognised.add("entry");
    warnings.push(`entry taken from the single-level ENTRY ZONE (${zoneLevel})`);
  }
  ...
  // a rejected one.
  if (out.entry_zone_low === null && trapZones.length > 0) {
    const fits = ({ low, high }) => {
      ...
      const stopClear =
        out.sl === null || out.direction === null
          ...
          : out.direction === "buy"
            ? out.sl < low
            : out.sl > high;
      return entryInside && stopClear;
    };
    const chosen = trapZones.find(fits);
    if (chosen) {
      out.entry_zone_low = chosen.low;
      out.entry_zone_high = chosen.high;
      recognised.add("entry_zone_low");
      recognised.add("entry_zone_high");
      warnings.push(`entry zone taken from the ${chosen.label} (${chosen.low} – ${chosen.high})`);
    } else {
      const seen = trapZones.map((z) => `${z.low} – ${z.high}`).join(", ");
      warnings.push(
        `KURTHI zone(s) ${seen} ignored: none sits consistently with entry ${out.entry} and stop ${out.sl}`,
      );
    }
  }

  // ----- §dol: the declared objective picks the target the R:R answers to -
  //
  // The analysis states its Draw On Liquidity as a level, and separately
  // lists the targets. When the DOL *is* one of those targets, that target
  // is the trade and the nearer ones are partials — so the floor has to be
  // measured there. Matched by level rather than by label text, because the
  // label is prose ("DOL Final / H1 EQL Strong Magnet") and the level is
  // not. A DOL matching nothing is left alone: guessing which target a
  // stray level meant would be worse than the TP1 default the operator
  // already expects.
  if (out.rr_target === null && dolLevel !== null) {
    const match = ["tp3", "tp2", "tp1"].find(
      (key) => out[key] !== null && Math.abs(out[key] - dolLevel) <= Math.abs(dolLevel) * 1e-6,
    );
    if (match) {
      out.rr_target = match;
      recognised.add("rr_target");
      if (match !== "tp1") {
        warnings.push(`R:R will be measured to ${match.toUpperCase()} — the declared DOL (${dolLevel})`);
      }
    } else {
      warnings.push(`DOL ${dolLevel} matches no declared target; R:R stays on TP1`);
    }
  }

  // ----- §rr: the declared ratio names the objective when the DOL cannot -
  //
  // The analysis writes its own ratio — "RR: 1:2.8" — and that number is a
  // second, independent statement of which target it considers the trade.
  // A setup whose TP1 is 1.06R and TP3 is 2.88R, declaring 1:2.8, has said
  // plainly that TP3 is the objective and TP1 is a scale-out; judging the
  // fill against TP1 would refuse a 2.8R setup for having decayed to 0.53R
  // against a target the analysis itself labelled "Low Hanging Fruit".
  //
  // Matched arithmetically against the levels already parsed, never on the
  // label prose, and only when the DOL line did not already settle it. A
  // ratio that matches nothing within tolerance is left alone: TP1 is the
  // default the operator expects, and a guess here is worse than it.
  if (out.rr_target === null && declaredRr !== null && out.entry !== null && out.sl !== null) {
    const risk = Math.abs(out.entry - out.sl);
    if (risk > 0) {
      const scored = ["tp1", "tp2", "tp3"]
        .filter((key) => out[key] !== null)
        .map((key) => ({ key, ratio: Math.abs(out[key] - out.entry) / risk }))
        .map((item) => ({ ...item, gap: Math.abs(item.ratio - declaredRr) }))
        .sort((a, b) => a.gap - b.gap)[0];
      // 15% of the declared ratio: wide enough for the rounding a chat
      // model does ("1:2.8" for 2.88R), far too narrow to reach TP1 at
      // 1.06R from a declared 2.8.
      if (scored && scored.gap <= declaredRr * 0.15) {
        out.rr_target = scored.key;
        recognised.add("rr_target");
        if (scored.key !== "tp1") {
          warnings.push(
            `R:R will be measured to ${scored.key.toUpperCase()} — the declared ${declaredRr}R matches it (${scored.ratio.toFixed(2)}R)`,
          );
        }
      }
    }
