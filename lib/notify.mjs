...
export class Notifier {
  constructor({
    ...
    maxAttempts = 5,
    now = () => Date.now(),
    // A test seam, and only that. Until this existed nothing could read
    // back what the operator is actually sent — the smoke suite stood up a
    // fake Telegram that the code could never reach, and asserted around
    // the gap. The default is the real endpoint; production never sets it.
  ...
  async deliver(text) {
    ...
    try {
      const response = await fetch(
        `${this.apiBase}/bot${this.token}/sendMessage`,
        {
          method: "POST",
